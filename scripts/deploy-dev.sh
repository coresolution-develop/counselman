...
LINKS_JAR="${ROOT_DIR}/build/deploy/dev/links-dev.jar"

echo "[2/3] Uploading to DEV server (${DEV_HOST})..."
ssh_cmd "mkdir -p ${CANCER_TREATMENT_APP_DIR} ${LINKS_APP_DIR}"
...
scp_cmd "${LINKS_JAR}" "${DEV_USER}@${DEV_HOST}:${LINKS_APP_DIR}/links.jar"

echo "[3/3] Restarting services..."
ssh_cmd "sudo systemctl restart mediplat && sudo systemctl restart cancer-treatment && sudo systemctl restart links"
...
echo "  Links (hub)      : http://${DEV_HOST}:8085/links"
