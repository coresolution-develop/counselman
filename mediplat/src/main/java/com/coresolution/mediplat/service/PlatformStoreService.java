...
public class PlatformStoreService {
    ...
    private void createMySqlTables() {
        ...
        jdbcTemplate.execute("""
                CREATE TABLE IF NOT EXISTS mp_login_audit (
                    id BIGINT NOT NULL AUTO_INCREMENT PRIMARY KEY,
                    inst_code VARCHAR(50) NOT NULL,
                    username VARCHAR(100) NOT NULL,
                    ...
                    session_seconds BIGINT NULL,
                    INDEX idx_mp_login_audit_inst_login (inst_code, login_at),
    ...
    private void createH2Tables() {
        ...
        jdbcTemplate.execute("""
                ...
                """);
        jdbcTemplate.execute("""
                CREATE INDEX IF NOT EXISTS idx_mp_login_audit_inst_login
                ON mp_login_audit (inst_code, login_at)
                """);
        jdbcTemplate.execute("""
    ...
    public Long recordLogin(PlatformSessionUser user) {
        if (user == null) {
            return null;
        }
        String instCode = normalizeInstCode(user.getInstCode());
        String username = normalizeUsername(user.getUsername());
        if (!StringUtils.hasText(instCode) || !StringUtils.hasText(username)) {
            return null;
        }
        String displayName = StringUtils.hasText(user.getDisplayName()) ? user.getDisplayName().trim() : username;
        String roleCode = StringUtils.hasText(user.getRoleCode()) ? user.getRoleCode().trim() : ROLE_USER;
        LocalDateTime now = LocalDateTime.now();
        KeyHolder keyHolder = new GeneratedKeyHolder();
        jdbcTemplate.update(connection -> {
            PreparedStatement ps = connection.prepareStatement("""
                    INSERT INTO mp_login_audit (inst_code, username, display_name, role_code, login_at)
                    VALUES (?, ?, ?, ?, ?)
    ...
    public void recordLogout(Long auditId) {
        ...
        Timestamp loginAt = jdbcTemplate.query(
                "SELECT login_at FROM mp_login_audit WHERE id = ? AND logout_at IS NULL",
                rs -> rs.next() ? rs.getTimestamp("login_at") : null,
                auditId);
        if (loginAt == null) {
            return;
        }
        LocalDateTime logoutAt = LocalDateTime.now();
        long seconds = Math.max(0L, Duration.between(loginAt.toLocalDateTime(), logoutAt).getSeconds());
        jdbcTemplate.update("""
                UPDATE mp_login_audit
                SET logout_at = ?, session_seconds = ?
                WHERE id = ? AND logout_at IS NULL
                """,
                Timestamp.valueOf(logoutAt),
                seconds,
                auditId);
    }

    public List<PlatformLoginAudit> listLoginAudits(
            String instCode,
            String username,
            LocalDate fromDate,
            LocalDate toDate,
        ...
        StringBuilder sql = new StringBuilder("""
                SELECT a.id, a.inst_code, COALESCE(i.inst_name, '') AS inst_name,
                       a.username, a.display_name, a.role_code,
                       a.login_at, a.logout_at, a.session_seconds
                FROM mp_login_audit a
                LEFT JOIN mp_institution i ON i.inst_code = a.inst_code
                WHERE 1=1
                """);
        List<Object> params = new ArrayList<>();
        String normalizedInstCode = normalizeInstCode(instCode);
        if (StringUtils.hasText(normalizedInstCode)) {
            sql.append(" AND a.inst_code = ?");
            params.add(normalizedInstCode);
        }
        String normalizedUsername = normalizeUsername(username);
        if (StringUtils.hasText(normalizedUsername)) {
            sql.append(" AND LOWER(a.username) LIKE ?");
            params.add("%" + normalizedUsername.toLowerCase(Locale.ROOT) + "%");
        }
        if (fromDate != null) {
            sql.append(" AND a.login_at >= ?");
        ...
        sql.append(" ORDER BY a.login_at DESC, a.id DESC LIMIT ").append(effectiveLimit);

        return jdbcTemplate.query(sql.toString(), (rs, rowNum) -> new PlatformLoginAudit(
                rs.getLong("id"),
                rs.getString("inst_code"),
                rs.getString("inst_name"),
                rs.getString("username"),
                ...
                toLocalDateTime(rs.getTimestamp("logout_at")),
                (Long) rs.getObject("session_seconds")), params.toArray());
    }
