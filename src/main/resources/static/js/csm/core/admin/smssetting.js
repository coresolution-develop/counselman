document.addEventListener("DOMContentLoaded", function () {
    ...
    let Open = document.getElementById('new-price');
    let Open2 = document.getElementsByClassName('templateFirstInsert');
    let modi = document.getElementById('modi-price');
    let TemplateName = '';
    let TemplateNote = '';
    let selectedIdx;
    let selectedsms;
	...
	tableRows.forEach(row => {
	    row.addEventListener('click', function() {
            ...
            selectedInst = this.getAttribute('data-inst-code');
            const cells = this.children;
            const selectedTemplateName = cells[0].textContent.trim();
            const selectedTemplateNote = cells[1].textContent.trim();
...
//            console.log(selectedTemplateName);
	        ...
	        selectedsms = this.getAttribute('data-sms');
	        selectedlms = this.getAttribute('data-lms');
	        selectedmms = this.getAttribute('data-mms');
//	        console.log("selectedsms", selectedsms);
//	        console.log("selectedlms", selectedlms);
//	        console.log("selectedmms", selectedmms);
			console.log(Inst);
	    });
	...
	if (Open) {
        Open.addEventListener('click', function () {
        	...
        	modalText.innerText = "전체 단가를 등록합니다.";
    ...
    if (modi) {
		modi.addEventListener('click', function() {
			console.log('개별 단가 수정');
			const modal3 = $('.modal5');
			const modalText = $('.menu_msg5')[0];
			if(selectedIdx) {
				modalText.innerText = '단가를 수정합니다.';
				modal3.addClass('show');
				$('.body').css('overflow', 'hidden');
				$('#smsprice').val(selectedsms);
				$('#lmsprice').val(selectedlms);
				$('#mmsprice').val(selectedmms);
				$('#smsprice').focus();
			} else {
				alert('기관을 먼저 선택해주세요.');
			}
	...
	$('#priceAllConfirmBtn').click(function() {
		priceAllInsert();
	});
	function priceAllInsert() {
		console.log($('#smspriceAll').val());
		console.log($('#lmspriceAll').val());
		console.log($('#mmspriceAll').val());
		const smspriceAll = $('#smspriceAll').val();
		const lmspriceAll = $('#lmspriceAll').val();
		const mmspriceAll = $('#mmspriceAll').val();
		const pricedata ={
			sms_price: smspriceAll,
			lms_price: lmspriceAll,
			mms_price: mmspriceAll,
			id_col_03: 'all'
		}
		spinner();
		$.ajax({
			url: '/csm/core/smssetting/priceInsert',
			type: 'POST',
			data: JSON.stringify(pricedata),
			contentType: 'application/json',
			success: function(response) {
				console.log("단가 등록 성공", response);
				closePopup3();
	...
	$('#priceConfirmBtn').click(function() {
		PriceModify();
	});
	function PriceModify() {
		console.log($('#smsprice').val());
		console.log($('#lmsprice').val());
		console.log($('#mmsprice').val());
		const smsprice = $('#smsprice').val();
		const lmsprice = $('#lmsprice').val();
		const mmsprice = $('#mmsprice').val();
		const pricedata ={
			sms_price: smsprice,
			lms_price: lmsprice,
			mms_price: mmsprice,
