...
<section class="normal">
	...
	<c:choose>
		...
		${pageContext.request.contextPath}/c/CounselList2.do
		</c:when>
		<c:otherwise>
		${pageContext.request.contextPath}/c/CounselListSearch2.do
		</c:otherwise>
	</c:choose>
	">
	<div class="flex flex-center " style="text-align: center; height: 88px; background-color: #dadada; font-size:20pt; color:#2f2f2f">
		...
		<a class="ip_sb" href="${pageContext.request.contextPath}/c/Written2.do" style="cursor: pointer;     display: flex;
				...
				<tr class="light list-tr" style="height: 78px; border-bottom: 1px solid #e9e9e9; color: #000000;">
					...
					<td>
						<div style="display: flex; align-items: center; vertical-align: middle; justify-content: center;;">
							<c:choose>
								<c:when test="${list.cs_data_30 == 1}">
									<div class="save flex flex-center" style="cursor: pointer;" onclick="location.href='${pageContext.request.contextPath}/c/WrittenModify2.do/${list.cs_idx}'">서명</div>
								</c:when>
								<c:otherwise>
				...
				<ul class="pagination justify-content-center" style="display: flex; justify-content: center; align-items: center;"id="pageInfo">
					...
					<c:choose>
						<c:when test="${pageMaker.cri.page != 1}">
							<li class="page-item"><a class="page-link" style="padding: 5px;"
								href="${pageContext.request.contextPath}/c/CounselList2.do?page=${pageMaker.startPage - pageMaker.startPage}&perPageNum=${pageMaker.cri.perPageNum}&type=${type}&keyword1=${keyword1}">&laquo;</a>
							</li>
						</c:when>
					</c:choose>
					<!-- 이전페이지로 이동하기 -->
					<c:choose>
						<c:when test="${pageMaker.prev == true}">
							<li class="page-item"><a class="page-link" style="padding: 5px;"
								href="${pageContext.request.contextPath}/c/CounselList2.do?page=${pageMaker.startPage - 1}&type=${type}&keyword1=${keyword1}">Previous</a>
							</li>
						</c:when>
					</c:choose>
					<!-- 리스트 페이징 -->
						...
						<c:choose>
							<c:when test="${num == pageMaker.cri.page}">
								<li class="page-item active"><a class="page-link" style="padding: 5px;"
									href="${pageContext.request.contextPath}/c/CounselList2.do?page=${num}&type=${type}&keyword1=${keyword1}">${num}</a>
								</li>
							</c:when>
							<c:otherwise>
								<li class="page-item"><a class="page-link" style="padding: 5px;"
					...
					<c:choose>
						<c:when test="${pageMaker.next == true}">
							<li class="page-item"><a class="page-link" style="padding: 5px;"
								href="${pageContext.request.contextPath}/c/CounselList2.do?page=${pageMaker.endPage + 1}&type=${type}&keyword1=${keyword1}">Next</a>
							</li>
						</c:when>
					</c:choose>
					...
					<c:choose>
						<c:when test="${pageMaker.cri.page < pageMaker.endPage}">
							<li class="page-item"><a class="page-link" style="padding: 5px;"
								href="${pageContext.request.contextPath}/c/CounselList2.do?page=${pageMaker.endPage}&perPageNum=${pageMaker.cri.perPageNum}&type=${type}&keyword1=${keyword1}">&raquo;</a>
							</li>
						</c:when>
