...
#### 🐛 알려진 버그
- [ ] **역할에 사용자 추가 시 후보 목록 필터 버그** — 역할에 사용자 1명을 추가한 뒤, 다시 "사용자 추가" 모달을 열면 **이미 해당 역할을 가진 사용자만** 후보로 노출됨. 정상 동작은 **역할을 아직 보유하지 않은 사용자**만 후보로 떠야 함. 모달 데이터 로딩 쿼리 또는 모달 상태 캐시 점검 필요 (`role-management.html` + `user_role_{inst}` 조인 조건)
  - 운영 확인 환경: `https://csm.sosyge.net/` (기존 csm) 또는 `http://115.68.177.207:18081/csm/` (csm-next)
  - 재현: 역할 관리 → 임의 역할의 사용자 추가 → 사용자 1명 추가 → 같은 역할 사용자 추가 모달 재오픈
