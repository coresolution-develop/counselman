...
### 📊 상담 리스트
- [x] 검색 기능 백엔드 연동 (환자명·보호자명·전화번호·상담자·상담내용 타입 선택 드롭다운 추가)
- [ ] 리스트 항목 설정 관리 UI — 보여줄/가릴 컬럼 선택, 좌측 고정 설정
- [x] 환자 정보 삭제 백엔드 연동 (체크박스 선택 + 다중 삭제)
- [x] 일괄 문자 보내기 백엔드 연동 (선택 행 수신자 자동 추출 + SMS/LMS 전송)
- [x] 상담 기록 카드 필터 (전체 / 오늘 / 미완료) — Alpine.js 클라이언트 필터링
