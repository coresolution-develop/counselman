# 무중단 배포 인프라 적용 순서 (Runbook) — P0 적용·검증 완료 / Step1~6 미적용
...
1. **본문 검토** (이 문서 확정)
2. **P0 단독** — 세션 외부화(DDL + 첫 JDBC WAR + 로그인/세션 검증). **이것만으로 03:00·배포 시 강제 로그아웃이 사라진다.** — ✅ **완료** (2026-07-03 jdbc WAR 배포, 2026-07-15 재기동 후 세션유지 확인)
3. **며칠 안정 관측** — P0 이상 없음 확인 후 진행 — ✅ 충족(2026-07-03 이후 무사고 가동)
...
> ✅ **적용·검증 완료 (2026-07-15 확인).** 운영 DB(`127.0.0.1:3306/csm`)에 `SPRING_SESSION`/`SPRING_SESSION_ATTRIBUTES` 존재·활성, 배포 WAR `store-type=jdbc`(build.time=2026-07-03), `systemctl restart csm-next` 후 세션 유지 확인.
> 아래 P0.1~P0.5 는 **최초 적용 절차의 기록**(재적용 불필요, 보존용).
>
