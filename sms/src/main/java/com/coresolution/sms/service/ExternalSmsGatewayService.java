...
/**
 * Bizppurio HTTP integration (token issuance + message send).
 * Ported from csm's ExternalSmsGatewayService — self-contained, depends only on
 * {@code sms.bizppurio.*} config. Kept byte-for-byte compatible so both apps behave
 * identically against the same Bizppurio account.
 */
@Service
